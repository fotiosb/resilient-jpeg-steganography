 ...
 */
{
	...
	if (free_code > maxcode) {
		...
		if (n_bits == MAX_LZW_BITS)
			maxcode = LZW_TABLE_SIZE;	/* free_code will
							 * never exceed this */
		else
 ...
 */
{
	...
	/*
	 * Probe hash table to see if a symbol exists for waiting_code
	 * followed by c. If so, replace waiting_code by that symbol and
	 * return. 
	 */
	i = ((hash_int) c << (MAX_LZW_BITS - 8)) + waiting_code;
	/*
	 * i is less than twice 2**MAX_LZW_BITS, therefore less than twice
	 * HSIZE 
	 */
	if (i >= HSIZE)
		i -= HSIZE;
	...
	/*
	 * here when hashtable[i] is an empty slot; desired symbol not in
	 * table 
	 */
	output(waiting_code);
 ...
 */
{
	...
	putc(0, dcinfo->output_file);	/* Reserved in GIF87 (aspect ratio in
					 * GIF89) */
	/*
	...
	for (i = 0; i < ColorMapSize; i++) {
		if (i < num_colors) {
			if (colormap != NULL) {
				...
				/*
				 * Create a gray-scale map of num_colors
				 * values, range 0..255 
				 */
				put_3bytes((i * 255 +
					    (num_colors -
