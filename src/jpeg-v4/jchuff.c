 ...
 */
{
	...
	/*
	 * We don't bother to fill in the decoding tables mincode[],
	 * maxcode[], 
	 */
	/*
...
{
	if (bytes_in_buffer)
		(*cinfo->methods->entropy_output) (cinfo, output_buffer,
						   bytes_in_buffer);
	bytes_in_buffer = 0;
}
 ...
 * left-justified in this part.  At most 16 bits can be passed to emit_bits in 
 * one call, and we never retain more than 7 bits in huff_put_buffer between
 * calls, so 24 bits are sufficient. 
 */
...
emit_bits(UINT16 code, int size)
{
	...
	put_buffer &= (((INT32) 1) << size) - 1;	/* Mask off any excess 
							 * bits in code */
...
huff_init(compress_info_ptr xinfo)
{
	...
	for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
		 ...
		 */
		/*
		 * We may do this more than once for same table, but it's not
		 * a big deal 
		 */
...
#define MAX_CLEN 32		/* assumed maximum initial code length */
	UINT8           bits[MAX_CLEN + 1];	/* bits[k] = # of symbols with 
						 * code length k */
	short           codesize[257];	/* codesize[k] = code length of symbol 
					 * k */
	short           others[257];	/* next symbol in current branch of
					 * tree */
	int             c1, c2;
	int             p, i, j;
	long            v;
	...
	for (;;) {
		...
		/*
		 * Find the next smallest nonzero frequency, set c2 = its
		 * symbol 
		 */
		/*
		 * In case of ties, take the larger symbol number 
	 ...
	 * that prefix length, a code word from the next shortest nonzero BITS 
	 * entry is converted into a prefix for two code words one bit longer. 
	 */
	...
	for (i = MAX_CLEN; i > 16; i--) {
		while (bits[i] > 0) {
			...
			bits[j + 1] += 2;	/* two new symbols in this
						 * length */
			bits[j]--;	/* symbol of this length is now a
					 * prefix */
		}
	}
...
htest_one_block(JBLOCK block, JCOEF block0, long dc_counts[], long ac_counts[])
{
	...
	for (k = 1; k < DCTSIZE2; k++) {
		if ((temp = block[k]) == 0) {
			...
			/*
			 * Count Huffman symbol for run length / number of
			 * bits 
			 */
			ac_counts[(r << 4) + nbits]++;
...
htest_encode(compress_info_ptr cinfo, JBLOCK * MCU_data)
{
	...
	for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
		...
		compptr = cinfo->cur_comp_info[ci];
		/*
		 * NB: unlike the real entropy encoder, we may not change the
		 * input data 
		 */
		htest_one_block(MCU_data[blkn],
				(JCOEF) (MCU_data[blkn][0] -
					 cinfo->last_dc_val[ci]),
 ...
 */
{
	...
	for (i = 0; i < cinfo->comps_in_scan; i++) {
		...
		if (dc_count_ptrs[tbl] == NULL) {
			dc_count_ptrs[tbl] =
			    (long *) (*cinfo->emethods->alloc_small)
			    (257 * SIZEOF(long));
	...
	for (tbl = 0; tbl < NUM_HUFF_TBLS; tbl++) {
		if (dc_count_ptrs[tbl] != NULL) {
			 ...
			 */
			(*cinfo->emethods->
			 free_small) ((void *) dc_count_ptrs[tbl]);
		}
		if (ac_count_ptrs[tbl] != NULL) {
			 ...
			 */
			(*cinfo->emethods->
			 free_small) ((void *) ac_count_ptrs[tbl]);
		}
	}
}
...
jselchuffman(compress_info_ptr cinfo)
{
	if (!cinfo->arith_code) {
		...
		/*
		 * The standard Huffman tables are only valid for 8-bit data
		 * precision. If the precision is higher, force optimization
		 * on so that usable tables will be computed.  This test can
		 * be removed if default tables are supplied that are valid
		 * for the desired precision. 
		 */
		if (cinfo->data_precision > 8)
			cinfo->optimize_coding = TRUE;
		if (cinfo->optimize_coding)
			cinfo->total_passes++;	/* one pass needed for entropy 
						 * optimization */
#endif
	}
}
