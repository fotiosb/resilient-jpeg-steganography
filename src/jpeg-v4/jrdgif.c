...
/*
 * LZW decompression tables: symbol_head[K] = prefix symbol of any LZW symbol
 * K (0..LZW_TABLE_SIZE-1) symbol_tail[K] = suffix byte of any LZW symbol K
 * (0..LZW_TABLE_SIZE-1) Note that entries 0..end_code of the above tables are 
 * not used, since those symbols represent raw bytes or special codes. The
 * stack represents the not-yet-used expansion of the last LZW symbol. In the
 * worst case, a symbol could expand to as many bytes as there are LZW
 * symbols, so we allocate LZW_TABLE_SIZE bytes for the stack. (This is
 * conservative since that number includes the raw-byte symbols.) The tables
 * are allocated from FAR heap space since they would use up rather a lot of
 * the near data space in a PC. 
 */
 ...
 */
METHODDEF void load_interlaced_image
PP((compress_info_ptr cinfo, JSAMPARRAY pixel_row));
METHODDEF void get_interlaced_row
PP((compress_info_ptr cinfo, JSAMPARRAY pixel_row));
 ...
 */
{
	...
	if ((cur_bit + code_size) > last_bit) {
		...
		/*
		 * preserve last two bytes of what we have -- assume code_size 
		 * <= 16 
		 */
 ...
 */
{
	...
	/*
	 * First time, just eat the expected Clear code(s) and return next
	 * code, 
	 */
	/*
	...
	if (first_time) {
		first_time = FALSE;
		code = clear_code;	/* enables sharing code with Clear
					 * case */
	} else {

		/*
		 * If any codes are stacked from a previously read symbol,
		 * return them 
		 */
		if (sp > symbol_stack)
			return (int) *(--sp);
	...
	if (code == clear_code) {
		...
		firstcode = oldcode = code;	/* make firstcode, oldcode
						 * valid! */
		return code;
	}
	...
	if (code == end_code) {
		/*
		 * Skip the rest of the image, unless GetCode already read
		 * terminator 
		 */
		if (!out_of_blocks) {
	...
	if (code >= max_code) {	/* special case for not-yet-defined symbol */
		...
		if (code > max_code) {
			WARNMS(cinfo->emethods, "Corrupt data in GIF file");
			incode = 0;	/* prevent creation of loops in symbol 
					 * table */
		}
		*sp++ = (UINT8) firstcode;	/* it will be defined as
						 * oldcode/firstcode */
		code = oldcode;
	}
	 ...
	 */
	while (code >= clear_code) {
		*sp++ = symbol_tail[code];	/* tail of symbol: a simple
						 * byte value */
		code = symbol_head[code];	/* head of symbol: another LZW 
						 * symbol */
	}
	/*
	...
	if ((code = max_code) < LZW_TABLE_SIZE) {
		/*
		 * Define a new symbol = prev sym + head of this sym's
		 * expansion 
		 */
 ...
 */
{
	...
	TRACEMS1(cinfo->emethods, 1,
		 "Ignoring GIF extension block of type 0x%02x", extlabel);
	/*
...
{
	char            hdrbuf[10];	/* workspace for reading control
					 * blocks */
	...
	if (hdrbuf[0] != 'G' || hdrbuf[1] != 'I' || hdrbuf[2] != 'F')
		ERREXIT(cinfo->emethods, "Not a GIF file");
	/*
	 * Check for expected version numbers. If unknown version, give
	 * warning and try to process anyway; this is per recommendation in
	 * GIF89a standard. 
	 */
	...
	/*
	 * we ignore the color resolution, sort flag, and background color
	 * index 
	 */
	...
	/*
	 * Scan until we reach start of desired image. We don't currently
	 * support skipping images, but could add it easily. 
	 */
	for (;;) {
		c = ReadByte(cinfo);
		...
		if (c != ',') {	/* Not an image separator? */
			TRACEMS1(cinfo->emethods, 1,
				 "Bogus input char 0x%02x, ignoring", c);
			continue;
		}
		 ...
		 */
		/*
		 * we'd need to skip rather than read colormap for ignored
		 * images 
		 */
		...
		input_code_size = ReadByte(cinfo);	/* get
							 * minimum-code-size
							 * byte */
		if (input_code_size < 2 || input_code_size >= MAX_LZW_BITS)
			ERREXIT1(cinfo->emethods, "Bogus codesize %d",
				 input_code_size);
	...
	if (is_interlaced) {
		/*
		 * We request the big array now, but can't access it until the 
		 * pipeline controller causes all the big arrays to be
		 * allocated.  Hence, the actual work of reading the image is
		 * postponed until the first call of get_input_row. 
		 */
...
load_interlaced_image(compress_info_ptr cinfo, JSAMPARRAY pixel_row)
{
	...
	for (row = 0; row < cinfo->image_height; row++) {
		(*cinfo->methods->progress_monitor) (cinfo, row,
						     cinfo->image_height);
