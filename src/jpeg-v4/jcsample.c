	       ...
	       JSAMPARRAY output_data)
{
	...
	if (output_rows != compptr->v_samp_factor ||
	    ...
	    input_cols * compptr->h_samp_factor !=
	    output_cols * cinfo->max_h_samp_factor)
		    ...
		    JSAMPARRAY output_data)
{
	...
	jcopy_sample_rows(input_data, 0, output_data, 0, output_rows,
			  output_cols);
}
...
h2v2_smooth_downsample(compress_info_ptr cinfo, int which_component,
		       long input_cols, int input_rows,
		       long output_cols, int output_rows,
		       JSAMPARRAY above, JSAMPARRAY input_data,
		       JSAMPARRAY below, JSAMPARRAY output_data)
{
	int             inrow, outrow;
	long            colctr;
	...
	/*
	 * We don't bother to form the individual "smoothed" input pixel
	 * values; we can directly compute the output which is the average of
	 * the four smoothed values.  Each of the four member pixels
	 * contributes a fraction (1-8*SF) to its own smoothed image and a
	 * fraction SF to each of the three other smoothed pixels, therefore a 
	 * total fraction (1-5*SF)/4 to the final output.  The four
	 * corner-adjacent neighbor pixels contribute a fraction SF to just
	 * one smoothed pixel, or SF/4 to the final output; while the eight
	 * edge-adjacent neighbors contribute SF to each of two smoothed
	 * pixels, or SF/2 overall.  In order to use integer arithmetic, these
	 * factors are scaled by 2^16 = 65536. Also recall that SF =
	 * smoothing_factor / 1024. 
	 */

	memberscale = 16384 - cinfo->smoothing_factor * 80;	/* scaled
								 * (1-5*SF)/4 */
	...
	for (outrow = 0; outrow < output_rows; outrow++) {
		...
		*outptr++ = (JSAMPLE) ((membersum + 32768L) >> 16);
		...
		for (colctr = output_cols - 2; colctr > 0; colctr--) {
			/*
			 * sum of pixels directly mapped to this output
			 * element 
			 */
			membersum =
	...
	for (outrow = 0; outrow < output_rows; outrow++) {
		...
		*outptr++ = (JSAMPLE) ((membersum + 32768L) >> 16);
		...
		for (colctr = output_cols - 2; colctr > 0; colctr--) {
			...
			below_ptr++;
			nextcolsum =
			    GETJSAMPLE(*above_ptr) + GETJSAMPLE(*below_ptr) +
			    GETJSAMPLE(*inptr);
			neighsum =
...
jseldownsample(compress_info_ptr cinfo)
{
	...
	if (cinfo->CCIR601_sampling)
		ERREXIT(cinfo->emethods,
			"CCIR601 downsampling not implemented yet");
	...
	for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
		...
		if (compptr->h_samp_factor == cinfo->max_h_samp_factor &&
			...
			ERREXIT(cinfo->emethods,
				"Fractional downsampling not implemented yet");
	}
