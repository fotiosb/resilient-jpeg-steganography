 ...
 */
{
	...
	/*
	 * We don't bother to fill in the encoding tables ehufco[] and
	 * ehufsi[], 
	 */
	/*
	...
	for (l = 1; l <= 16; l++) {
		if (htbl->bits[l]) {
			htbl->valptr[l] = p;	/* huffval[] index of 1st sym
						 * of code len l */
			htbl->mincode[l] = huffcode[p];	/* minimum code of
							 * length l */
			p += htbl->bits[l];
			htbl->maxcode[l] = huffcode[p - 1];	/* maximum
								 * code of
								 * length l */
		} else {
			htbl->maxcode[l] = -1;
 ...
 */
{
	...
	while (bits_left < MIN_GET_BITS) {
		...
		if (c == 0xFF) {
			...
			if (c2 != 0) {
				/*
				 * Oops, it's actually a marker indicating end 
				 * of compressed data. 
				 */
				/*
				...
				JUNGETC(c, dcinfo);
				/*
				 * There should be enough bits still left in
				 * the data segment; 
				 */
				/*
				 ...
				 * stuff zeroes into the data stream, so we
				 * can produce some kind of image. Note that
				 * this will be repeated for each byte
				 * demanded for the rest of the segment; this
				 * is a bit slow but not unreasonably so. The
				 * main thing is to avoid getting a zillion
				 * warnings, hence: 
				 */
				if (!printed_eod) {
					WARNMS(dcinfo->emethods,
	...
	/*
	 * Having filled get_buffer, extract desired bits (this simplifies
	 * macros) 
	 */
	bits_left -= nbits;
 ...
 * reference values beyond the end of the array.  To avoid a wild store, we
 * put some extra zeroes after the real entries. 
 */
