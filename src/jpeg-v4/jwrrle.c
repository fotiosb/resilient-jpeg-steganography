...
Sorry, this code only copes with 8 - bit JSAMPLEs.	/* deliberate syntax
							 * err */
#endif
    /*
...
static big_sarray_ptr channels[MAX_CHANS];	/* Virtual arrays for saved
						 * data */
...
static rle_map *output_colormap;	/* RLE-style color map, or NULL if
					 * none */
...
put_color_map(decompress_info_ptr cinfo, int num_colors, JSAMPARRAY colormap)
{
	...
	if (num_colors > CMAPLENGTH)
		ERREXIT1(cinfo->emethods,
			 "Cannot handle %d colormap entries for RLE",
			 num_colors);
	...
	output_colormap =
	    (rle_map *) (*cinfo->emethods->alloc_small) (cmapsize);
	...
	for (ci = 0; ci < cinfo->color_out_comps; ci++) {
		for (i = 0; i < num_colors; i++) {
			output_colormap[ci * CMAPLENGTH + i] =
			    GETJSAMPLE(colormap[ci][i]) << 8;
		}
	}
...
output_term(decompress_info_ptr cinfo)
{
	...
	/*
	 * Now output the RLE data from our virtual array(s). We assume here
	 * that (a) rle_pixel is represented the same as JSAMPLE, and (b) we
	 * are not on a machine where FAR pointers differ from regular. 
	 */
