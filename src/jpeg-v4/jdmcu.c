			       ...
			       JBLOCKIMAGE image_data)
{
	...
	/*
	 * this is pretty easy since there is one component and one block per
	 * MCU 
	 */
...
METHODDEF void
disassemble_interleaved_MCU(decompress_info_ptr cinfo, JBLOCKIMAGE image_data)
{
	...
	for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
		...
		for (ri = 0; ri < numrows; ri++) {
			...
			for (bi = 0; bi < blocksperrow; bi++) {
				/*
				 * copy the data into a local DCTBLOCK.  This
				 * allows for change of representation (if
				 * DCTELEM != JCOEF).  On 80x86 machines it
				 * also brings the data back from FAR storage
				 * to NEAR storage. 
				 */
				{
					register JCOEFPTR elemptr =
					    browptr[bi];
				...
				/*
				 * Output the data into the sample array. Note 
				 * change from signed to unsigned
				 * representation: DCT calculation works with
				 * values +-CENTERJSAMPLE, but sample arrays
				 * always hold 0..MAXJSAMPLE. We have to do
				 * range-limiting because of quantization
				 * errors in the DCT/IDCT phase.  We use the
				 * sample_range_limit[] table to do this
				 * quickly; the CENTERJSAMPLE offset is folded 
				 * into table indexing. 
				 */
				{
...
jseldmcu(decompress_info_ptr cinfo)
{
	if (cinfo->comps_in_scan == 1)
		cinfo->methods->disassemble_MCU =
		    disassemble_noninterleaved_MCU;
	else
