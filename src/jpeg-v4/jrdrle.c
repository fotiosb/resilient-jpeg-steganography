...
static big_sarray_ptr red_channel;	/* three arrays for
...
input_init(compress_info_ptr cinfo)
{
	...
	if (header.ncolors == 1 && header.ncmap == 0) {
		...
		TRACEMS1(cinfo->emethods, 1,
			 "Colormapped RLE file with map of length %d",
			 1 << header.cmaplen);
	} else if (header.ncolors == 3 && header.ncmap == 3) {
		visual = TRUECOLOR;
		colormap = header.cmap;
		TRACEMS1(cinfo->emethods, 1,
			 "Full-color RLE file with map of length %d",
			 1 << header.cmaplen);
	...
	case GRAYSCALE:
		 ...
		 */
		image =
		    (*cinfo->emethods->request_big_sarray) (width, height, 1L);
...
load_image(compress_info_ptr cinfo, JSAMPARRAY pixel_row)
{
	...
	/*
	 * Read the RLE data into our virtual array(s). We assume here that
	 * (a) rle_pixel is represented the same as JSAMPLE, and (b) we are
	 * not on a machine where FAR pointers differ from regular. 
	 */
