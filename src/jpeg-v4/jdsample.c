	     ...
	     JSAMPARRAY output_data)
{
	...
	if (input_rows != compptr->v_samp_factor ||
	    ...
	    output_cols * compptr->h_samp_factor !=
	    input_cols * cinfo->max_h_samp_factor)
	...
	for (inrow = 0; inrow < input_rows; inrow++) {
		...
		*outptr++ =
		...
		for (colctr = input_cols - 2; colctr > 0; colctr--) {
			...
			*outptr++ =
			    (JSAMPLE) ((invalue + GETJSAMPLE(inptr[-2]) +
					2) >> 2);
			*outptr++ =
			    (JSAMPLE) ((invalue + GETJSAMPLE(*inptr) +
					2) >> 2);
		}
		...
		invalue = GETJSAMPLE(*inptr);
		*outptr++ =
		    (JSAMPLE) ((invalue * 3 + GETJSAMPLE(inptr[-1]) + 2) >> 2);
		*outptr++ = (JSAMPLE) invalue;
	}
}
	      ...
	      JSAMPARRAY output_data)
{
	...
	for (inrow = 0; inrow < input_rows; inrow++) {
		for (v = 0; v < 2; v++) {
			/*
			 * inptr0 points to nearest input row, inptr1 points
			 * to next nearest 
			 */
			...
			*outptr++ =
			    (JSAMPLE) ((thiscolsum * 3 + nextcolsum + 8) >> 4);
			...
			for (colctr = input_cols - 2; colctr > 0; colctr--) {
				...
				nextcolsum =
				    ...
				    GETJSAMPLE(*inptr1++);
				*outptr++ =
				    (JSAMPLE) ((thiscolsum * 3 + lastcolsum +
						8) >> 4);
				*outptr++ =
				    (JSAMPLE) ((thiscolsum * 3 + nextcolsum +
						8) >> 4);
				lastcolsum = thiscolsum;
				thiscolsum = nextcolsum;
			}
