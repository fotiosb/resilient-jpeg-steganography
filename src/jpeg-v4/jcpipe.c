 ...
 */
{
	...
	if (cinfo->restart_in_rows > 0) {
		long            nominal =
		    cinfo->restart_in_rows * cinfo->MCUs_per_row;
 ...
 */
{
	...
	for (ci = 0; ci < cinfo->num_components; ci++) {
		...
		fullsize_data[0][ci] = (*cinfo->emethods->alloc_small_sarray)
		    (fullsize_width, (long) (vs * (DCTSIZE + 2)));
		/*
		 ...
		 */
		for (i = 0; i < vs * 2; i++) {
			fullsize_data[1][ci][vs * DCTSIZE + i] =
			    fullsize_data[0][ci][vs * (DCTSIZE - 2) + i];
			fullsize_data[1][ci][vs * (DCTSIZE - 2) + i] =
			    fullsize_data[0][ci][vs * DCTSIZE + i];
		}
	}
}
 ...
 */
{
	...
	for (ci = 0; ci < cinfo->num_components; ci++) {
		...
		if (below >= 0)
			...
			for (i = 0; i < vs; i++)
				dummy[i] =
				    fullsize_data[ci][(current + 1) * vs - 1];
 ...
 * MCU_output_catcher. To avoid a lot of row-pointer overhead, we cram as many 
 * MCUs into each row of whole_scan_MCUs as we can get without exceeding
 * 32Kbytes per row. NOTE: the "arbitrary" constant here must not exceed
 * MAX_ALLOC_CHUNK defined in jmemsys.h, which is 64K-epsilon in most DOS
 * implementations. 
 */

#define MAX_WHOLE_ROW_BLOCKS	((int) (32768L / SIZEOF(JBLOCK)))	/* max 
									 * blocks/row 
									 */

static big_barray_ptr whole_scan_MCUs;	/* Big array for saving the MCUs */
static int      MCUs_in_big_row;	/* # of MCUs in each row of
					 * whole_scan_MCUs */
 ...
 */
{
	...
	if (next_MCU_index >= MCUs_in_big_row) {
		rowptr =
		    (*cinfo->emethods->access_big_barray) (whole_scan_MCUs,
							   next_whole_row,
							   TRUE);
		next_whole_row++;
		next_MCU_index = 0;
...
{
	/*
	 * On an 80x86 machine, the entropy encoder expects the passed data
	 * block to be in NEAR memory (for performance reasons), so we have to 
	 * copy it back from the big array to a local array.  On less
	 * brain-damaged CPUs we needn't do that. 
	 */
	...
	int             next_index;
	JBLOCKARRAY     rowptr = NULL;	/* init only to suppress compiler
					 * complaint */
	...
	for (mcurow = 0; mcurow < cinfo->MCU_rows_in_scan; mcurow++) {
		...
		for (mcuindex = 0; mcuindex < cinfo->MCUs_per_row; mcuindex++) {
			if (next_index >= MCUs_in_big_row) {
				rowptr =
				    (*cinfo->emethods->
				     access_big_barray) (whole_scan_MCUs,
							 next_row, FALSE);
				next_row++;
				next_index = 0;
			}
#ifdef NEED_FAR_POINTERS
			jcopy_block_row(rowptr[0] + next_index * cinfo->blocks_in_MCU, (JBLOCKROW) MCU_data,	/* casts 
														 * near 
														 * to 
														 * far 
														 * pointer! 
			...
			(*output_method) (cinfo,
					  rowptr[0] +
					  next_index * cinfo->blocks_in_MCU);
#endif
			next_index++;
		}
	}
...
single_ccontroller(compress_info_ptr cinfo)
{
	 ...
	 * input file reader) may need one. 
	 */
	...
	/*
	 * Loop over input image: rows_in_mem pixel rows are processed per
	 * loop 
	 */
	...
	for (cur_pixel_row = 0; cur_pixel_row < cinfo->image_height;
		...
		/*
		 * Obtain rows_this_time pixel rows and expand to rows_in_mem
		 * rows. 
		 */
		/*
		...
		if (cur_pixel_row) {
			 ...
			 */
			downsample(cinfo, fullsize_data[whichss], sampled_data,
		...
		/*
		 * Downsample second through next-to-last row groups of this
		 * set 
		 */
	...
	downsample(cinfo, fullsize_data[whichss], sampled_data, fullsize_width,
		   ...
		   (short) (DCTSIZE - 1));
	/*
	 * Dump the remaining data (may be less than full height if
	 * uninterleaved) 
	 */
	(*cinfo->methods->extract_MCUs) (cinfo, sampled_data,
					 (int) (cinfo->MCU_rows_in_scan -
						mcu_rows_output),
...
single_eopt_ccontroller(compress_info_ptr cinfo)
{
	...
	cinfo->total_passes += 2;	/* entropy encoder must add # passes
					 * it uses */
	...
	/*
	 * Request a big array: whole_scan_MCUs saves the MCU data for the
	 * scan 
	 */
	whole_scan_MCUs = (*cinfo->emethods->request_big_barray)
	    ((long) blocks_in_big_row, (long) (cinfo->MCUs_per_row * cinfo->MCU_rows_in_scan + MCUs_in_big_row - 1) / MCUs_in_big_row, 1L);	/* unit 
																		 * height 
																		 * is 
																		 * 1 
																		 * row 
	...
	next_MCU_index = MCUs_in_big_row;	/* forces access on first
						 * call! */
	 ...
	 */
	/*
	 * Note: we can't do write_scan_header until entropy parameters are
	 * set! 
	 */
...
jselcpipeline(compress_info_ptr cinfo)
{
	if (cinfo->interleave || cinfo->num_components == 1) {
		...
		if (cinfo->optimize_coding)
			cinfo->methods->c_pipeline_controller =
			    single_eopt_ccontroller;
		else
#endif
			cinfo->methods->c_pipeline_controller =
			    single_ccontroller;
