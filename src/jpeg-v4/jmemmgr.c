...
/*
 * These macros are the assumed overhead per block for malloc(). They don't
 * have to be accurate, but the printed statistics will be off a little bit if 
 * they are not. 
 */
...
print_mem_stats(void)
{
	 ...
	 * trace message mechanism... helpful 'cuz trace_message can't handle
	 * longs. 
	 */
 ...
 */
{
	...
	if (methods->trace_level <= 0)	/* don't do it if free_all() will */
		print_mem_stats();	/* print optional memory usage
					 * statistics */
#endif
 ...
 */
{
	...
	/*
	 * Get space for header and row pointers; this is always "near" on
	 * 80x86 
	 */
	hdr =
	    (small_sarray_ptr) alloc_small((size_t) (numrows * SIZEOF(JSAMPROW)
						     +
						     SIZEOF
						     (small_sarray_hdr)));
...
#ifdef MEM_STATS
		total_bytes_sarray +=
		    rowsperchunk * samplesperrow * SIZEOF(JSAMPLE)
		    + MALLOC_FAR_OVERHEAD;
#endif
		workspace =
		    (JSAMPROW)
 ...
 */
{
	...
	hdr =
	    (small_barray_ptr)
	    alloc_small((size_t) (numrows * SIZEOF(JBLOCKROW)
...
#ifdef MEM_STATS
		total_bytes_barray +=
		    rowsperchunk * blocksperrow * SIZEOF(JBLOCK)
		    + MALLOC_FAR_OVERHEAD;
#endif
		workspace =
		    (JBLOCKROW)
...
/*
 * The control blocks for virtual arrays. System-dependent info for the
 * associated backing store is hidden inside the backing_store_info struct. 
 */

struct big_sarray_control {
	long            rows_in_array;	/* total virtual array height */
	long            samplesperrow;	/* width of array (and of memory
					 * buffer) */
	long            unitheight;	/* # of rows accessed by
					 * access_big_sarray() */
...
struct big_barray_control {
	long            rows_in_array;	/* total virtual array height */
	long            blocksperrow;	/* width of array (and of memory
					 * buffer) */
	long            unitheight;	/* # of rows accessed by
					 * access_big_barray() */
 ...
 */
{
	 ...
	 */
	result =
	    (big_sarray_ptr) alloc_small(SIZEOF(struct big_sarray_control));
 ...
 */
{
	 ...
	 */
	result =
	    (big_barray_ptr) alloc_small(SIZEOF(struct big_barray_control));
 ...
 */
{
	long            total_extra_space =
	    extra_small_samples * SIZEOF(JSAMPLE)
	...
	/*
	 * Determine amount of memory to actually use; this is
	 * system-dependent. 
	 */
	...
	if (avail_mem >= maximum_space)
		...
		max_unitheights =
		    (avail_mem - total_extra_space) / space_per_unitheight;
		/*
		 * If there doesn't seem to be enough space, try to get the
		 * minimum anyway.  This allows a "stub" implementation of
		 * jmem_available(). 
		 */
	...
	for (sptr = big_sarray_list; sptr != NULL; sptr = sptr->next) {
		if (sptr->mem_buffer == NULL) {	/* if not realized yet */
			unitheights =
			    (sptr->rows_in_array + sptr->unitheight - 1L)
			    / sptr->unitheight;
			if (unitheights <= max_unitheights) {
				 ...
				 */
				sptr->rows_in_mem =
				    max_unitheights * sptr->unitheight;
			...
			sptr->mem_buffer =
			    alloc_small_sarray(sptr->samplesperrow,
					       sptr->rows_in_mem);
			/*
			 * Reach into the small_sarray header and get the
			 * rowsperchunk field. Yes, I know, this is horrible
			 * coding practice. 
			 */
			sptr->rowsperchunk =
			    ((small_sarray_ptr) sptr->mem_buffer)[-1].
			    rowsperchunk;
	...
	for (bptr = big_barray_list; bptr != NULL; bptr = bptr->next) {
		if (bptr->mem_buffer == NULL) {	/* if not realized yet */
			unitheights =
			    (bptr->rows_in_array + bptr->unitheight - 1L)
			    / bptr->unitheight;
			if (unitheights <= max_unitheights) {
				/*
				 * This buffer fits in memory 
				 ...
				 */
				bptr->rows_in_mem =
				    max_unitheights * bptr->unitheight;
			...
			bptr->mem_buffer =
			    alloc_small_barray(bptr->blocksperrow,
					       bptr->rows_in_mem);
			/*
			 * Reach into the small_barray header and get the
			 * rowsperchunk field. 
			 */
			bptr->rowsperchunk =
			    ((small_barray_ptr) bptr->mem_buffer)[-1].
			    rowsperchunk;
 ...
 */
{
	...
	for (i = 0; i < ptr->rows_in_mem; i += ptr->rowsperchunk) {
		 ...
		 */
		rows =
		    MIN(rows, ptr->rows_in_array - (ptr->cur_start_row + i));
 ...
 */
{
	...
	if (start_row < ptr->cur_start_row ||
		...
		/*
		 * Decide what part of virtual array to access. Algorithm: if
		 * target address > current window, assume forward scan, load
		 * starting at target address.  If target address < current
		 * window, assume backward scan, load so that target address
		 * is top of window. Note that when switching from forward
		 * write to forward read, will have start_row = 0, so the
		 * limiting case applies and we load from 0 anyway. 
		 */
		if (start_row > ptr->cur_start_row) {
			ptr->cur_start_row = start_row;
		} else {
			ptr->cur_start_row =
			    start_row + ptr->unitheight - ptr->rows_in_mem;
			if (ptr->cur_start_row < 0)
				ptr->cur_start_row = 0;	/* don't fall off
							 * front end of file */
		}
		/*
		 * If reading, read in the selected part of the array. If we
		 * are writing, we need not pre-read the selected portion,
		 * since the access sequence constraints ensure it would be
		 * garbage. 
		 */
		if (!writable) {
...
jselmemmgr(external_methods_ptr emethods)
{
	 ...
	 * default max_memory setting from jmem_init.  Note that a command
	 * line -m argument may again override this value. If your system
	 * doesn't support getenv(), define NO_GETENV to disable this feature. 
	 */
#ifndef NO_GETENV
	{
		char           *memenv;
