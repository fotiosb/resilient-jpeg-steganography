...
static int      dctcoefcount;	/* This will probably overflow on a 16-bit-int 
				 * machine */
#endif
 ...
 */
{
...

	{
		...
		for (elemr = DCTSIZE; elemr > 0; elemr--) {
			...
			*localblkptr++ =
			    (DCTELEM) (GETJSAMPLE(*elemptr++) - CENTERJSAMPLE);
			*localblkptr++ =
			    (DCTELEM) (GETJSAMPLE(*elemptr++) - CENTERJSAMPLE);
			*localblkptr++ =
			    (DCTELEM) (GETJSAMPLE(*elemptr++) - CENTERJSAMPLE);
...
extract_MCUs(compress_info_ptr cinfo,
	     ...
	     int num_mcu_rows, MCU_output_method_ptr output_method)
{
	...
	for (mcurow = 0; mcurow < num_mcu_rows; mcurow++) {
		for (mcuindex = 0; mcuindex < cinfo->MCUs_per_row; mcuindex++) {
			/*
			 * Extract data from the image array, DCT it, and
			 * quantize it 
			 */
			blkn = 0;
			for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
				...
				quant_ptr =
				    cinfo->quant_tbl_ptrs[compptr->
							  quant_tbl_no];
				for (ypos = 0; ypos < compptr->MCU_height;
				     ypos++) {
					for (xpos = 0;
			...
			/*
			 * Send the MCU whereever the pipeline controller
			 * wants it to go 
			 */
