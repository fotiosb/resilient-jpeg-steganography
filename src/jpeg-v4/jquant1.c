 ...
 * rows. In each of the xxxrowerrs[] arrays, indexing is
 * [component#][position]. We provide (#columns + 2) entries per component;
 * the extra entry at each end saves us from special-casing the first and last 
 * pixels. In evenrowerrs[], the entries for a component are stored
 * left-to-right, but in oddrowerrs[] they are stored right-to-left.  This
 * means we always process the current row's error entries in increasing order 
 * and the next row's error entries in decreasing order, regardless of whether 
 * we are working L-to-R or R-to-L in the pixel data! Note: on a wide image,
 * we might not have enough room in a PC's near data segment to hold the error 
 * arrays; so they are allocated with alloc_medium. 
 */
 ...
 */
{
	int             nc = cinfo->color_out_comps;	/* number of color
							 * components */
	...
	/*
	 * We can allocate at least the nc'th root of max_colors per
	 * component. 
	 */
	/*
	...
	do {
		...
		for (i = 1; i < nc; i++)
			temp *= iroot;
	} while (temp <= (long) max_colors);	/* repeat till iroot exceeds
						 * root */
	...
	if (cinfo->out_color_space == CS_RGB && nc == 3) {
		/*
		 * We provide a special policy for quantizing in RGB space. If 
		 * 256 colors are requested, we allocate 8 red, 8 green, 4
		 * blue levels; this corresponds to the common 3/3/2-bit
		 * scheme.  For other totals, the counts are set so that the
		 * number of colors allocated to each component are roughly in 
		 * the proportion R 3, G 4, B 2. For low color counts, it's
		 * easier to hardwire the optimal choices than try to tweak
		 * the algorithm to generate them. 
		 */
		if (max_colors == 256) {
			Ncolors[0] = 8;
			Ncolors[1] = 8;
		...
		if (max_colors < 12) {
			...
			/*
			 * these weights are readily derived with a little
			 * algebra 
			 */
			Ncolors[0] = (iroot * 266) >> 8;	/* R weight is 
								 * 1.0400 */
			Ncolors[1] = (iroot * 355) >> 8;	/* G weight is 
								 * 1.3867 */
			Ncolors[2] = (iroot * 177) >> 8;	/* B weight is 
								 * 0.6934 */
		}
		total_colors = Ncolors[0] * Ncolors[1] * Ncolors[2];
		/*
		 * The above computation produces "floor" values, so we may be 
		 * able to increment the count for one or more components
		 * without exceeding max_colors.  We try in the order B, G, R. 
		 */
		do {
			changed = FALSE;
			for (i = 2; i >= 0; i--) {
				/*
				 * calculate new total_colors if Ncolors[i] is 
				 * incremented 
				 */
				temp = total_colors / Ncolors[i];
				temp *= Ncolors[i] + 1;	/* done in long arith
							 * to avoid oflo */
				if (temp <= (long) max_colors) {
					Ncolors[i]++;	/* OK, apply the
							 * increment */
		...
		/*
		 * For any colorspace besides RGB, treat all the components
		 * equally. 
		 */
		...
		/*
		 * We may be able to increment the count for one or more
		 * components without exceeding max_colors, though we know not 
		 * all can be incremented. 
		 */
		for (i = 0; i < nc; i++) {
			/*
			 * calculate new total_colors if Ncolors[i] is
			 ...
			 */
			temp = total_colors / Ncolors[i];
			temp *= Ncolors[i] + 1;	/* done in long arith to avoid 
						 * oflo */
			if (temp > (long) max_colors)
...
{
	/*
	 * We always provide values 0 and MAXJSAMPLE for each component; any
	 * additional values are equally spaced between these limits. (Forcing 
	 * the upper and lower values to the limits ensures that dithering
	 * can't produce a color outside the selected gamut.) 
	 */
...
color_quant_init(decompress_info_ptr cinfo)
{
	...
	int             Ncolors[MAX_COMPONENTS];	/* # of values alloced 
							 * to each component */
	...
	if (cinfo->color_out_comps == 3)
		TRACEMS4(cinfo->emethods, 1,
			 "Quantizing to %d = %d*%d*%d colors", total_colors,
			 Ncolors[0], Ncolors[1], Ncolors[2]);
	else
	...
	for (i = 0; i < cinfo->color_out_comps; i++) {
		...
		for (j = 0; j < nci; j++) {
			/*
			 * Compute j'th output value (out of nci) for
			 * component 
			 */
			val = output_value(cinfo, i, j, nci - 1);
			/*
			 * Fill in all colormap entries that have this value
			 * of this component 
			 */
		...
		blkdist = blksize;	/* blksize of this color is blkdist of 
					 * next */
		...
		for (j = 0; j <= MAXJSAMPLE; j++) {
			while (j > k)	/* advance val if past boundary */
				k = largest_input_value(cinfo, i, ++val,
							nci - 1);
			/*
			 * premultiply so that no multiplication needed in
			 * main processing 
			 */
	...
	if (cinfo->use_dithering) {
		size_t          arraysize =
		    (size_t) ((cinfo->image_width + 2L) * SIZEOF(FSERROR));

		for (i = 0; i < cinfo->color_out_comps; i++) {
			evenrowerrs[i] =
			    (FSERRPTR) (*cinfo->emethods->
					alloc_medium) (arraysize);
			oddrowerrs[i] =
			    (FSERRPTR) (*cinfo->emethods->
					alloc_medium) (arraysize);
			/*
			 * we only need to zero the forward contribution for
			 * current row. 
			 */
 ...
 */
{
	...
	for (row = 0; row < num_rows; row++) {
		...
		for (col = width; col > 0; col--) {
			pixcode =
			    GETJSAMPLE(colorindex[0][GETJSAMPLE(*ptr0++)]);
			pixcode +=
			    GETJSAMPLE(colorindex[1][GETJSAMPLE(*ptr1++)]);
			pixcode +=
			    GETJSAMPLE(colorindex[2][GETJSAMPLE(*ptr2++)]);
			*ptrout++ = (JSAMPLE) pixcode;
		}
	}
}
 ...
 */
{
	...
	SHIFT_TEMPS for (row = 0; row < num_rows; row++) {
		...
		for (ci = 0; ci < nc; ci++) {
			...
			*nextrowerr = 0;	/* need only initialize this
						 * one entry */
			for (col_counter = width; col_counter > 0;
			     col_counter--) {
				/*
				 * Get accumulated error for this component,
				 * round to integer. RIGHT_SHIFT rounds
				 * towards minus infinity, so adding 8 is
				 * correct for either sign of the error value. 
				 */
				val = RIGHT_SHIFT(*thisrowerr + 8, 4);
				/*
				 * Compute pixel value + error compensation,
				 * range-limit to 0..MAXJSAMPLE.  Note max
				 * error value is +- MAXJSAMPLE. 
				 */
				val =
				    GETJSAMPLE(range_limit
					       [GETJSAMPLE(*input_ptr) + val]);
				/*
				 * Select output value, accumulate into output 
				 * code for this pixel 
				 */
				pixcode =
				    GETJSAMPLE(*output_ptr) +
				    GETJSAMPLE(colorindex_ci[val]);
				*output_ptr = (JSAMPLE) pixcode;
				 ...
				 */
				/*
				 * value of pixcode, because the colormap is
				 * orthogonal. 
				 */
				val -= GETJSAMPLE(colormap_ci[pixcode]);
				/*
				 * Propagate error to (same component of)
				 * adjacent pixels 
				 */
				/*
				 * Remember that nextrowerr entries are in
				 * reverse order! 
				 */
				two_val = val * 2;
				nextrowerr[-1] = val;	/* not +=, since not
							 * initialized yet */
				...
				thisrowerr[1] += val;
				input_ptr += dir;	/* advance input ptr
							 * to next column */
				output_ptr += dir;	/* advance output ptr
							 * to next column */
				thisrowerr++;	/* cur-row error ptr advances
						 * to right */
				nextrowerr--;	/* next-row error ptr advances 
						 * to left */
			}
		}
