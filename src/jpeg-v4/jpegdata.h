 ...
 * these types may need work.  You can replace them with some kind of pointer
 * or indicator that is useful to you, or just ignore 'em. Note that the user
 * interface and the various jrdxxx/jwrxxx modules will also need work for
 * non-stdio input/output. 
 */
 ...
 * You could modify them if you need to change function linkage conventions, as 
 * is shown below for use with C++.  Another application would be to make all
 * functions global for use with code profilers that require it. NOTE: the C++
 * test does the right thing if you are reading this include file in a C++
 * application to link to JPEG code that's been compiled with a regular C
 * compiler.  I'm not sure it works if you try to compile the JPEG code with
 * C++. 
 */
...
/*
 * Here is the pseudo-keyword for declaring pointers that must be "far" on
 * 80x86 machines.  Most of the specialized coding for 80x86 is handled by
 * just saying "FAR *" where such a pointer is needed.  In a few places
...
/*
 * The input and output data of the DCT transform subroutines are of the
 * following type, which need not be the same as JCOEF. For example, on a
 * machine with fast floating point, it might make sense to recode the DCT
 * routines to use floating point; then DCTELEM would be 'float' or 'double'. 
 */
...
typedef struct {		/* Basic info about one component */
	 ...
	 */
	short           component_id;	/* identifier for this component
					 * (0..255) */
	short           component_index;	/* its index in SOF or
						 * cinfo->comp_info[] */
	 ...
	 * startup 
	 */
	long            true_comp_width;	/* component's image width in
						 * samples */
	long            true_comp_height;	/* component's image height in 
						 * samples */
	/*
	 ...
	 */
	short           MCU_width;	/* number of blocks per MCU,
					 * horizontally */
	short           MCU_height;	/* number of blocks per MCU,
					 * vertically */
	short           MCU_blocks;	/* MCU_width * MCU_height */
	long            downsampled_width;	/* image width in samples,
						 * after expansion */
	long            downsampled_height;	/* image height in samples,
						 * after expansion */
	/*
...
/*
 * DCT coefficient quantization tables. For 8-bit precision, 'INT16' should be 
 * good enough for quantization values; for more precision, we go for the full 
 * 16 bits.  'INT16' provides a useful speedup on many machines
 * (multiplication & division of JCOEFs by quantization values is a
 * significant chunk of the runtime). Note: the values in a QUANT_TBL are
 * always given in zigzag order. 
 */
...
typedef struct {		/* A Huffman coding table */
	 ...
	 * marker 
	 */
	UINT8           bits[17];	/* bits[k] = # of symbols with codes
					 * of */
	/*
	 * length k bits; bits[0] is unused 
	 */
	UINT8           huffval[256];	/* The symbols, in order of incr code
					 * length */
	/*
	 * This field is used only during compression.  It's initialized FALSE 
	 * when the table is created, and set TRUE when it's been output to
	 * the file. 
	 */
	...
	INT32           maxcode[18];	/* largest code of length k (-1 if
					 * none) */
	/*
	 * (maxcode[17] is a sentinel to ensure huff_DECODE terminates) 
	 */
	short           valptr[17];	/* huffval[] index of 1st symbol of
					 * length k */
} HUFF_TBL;
...
struct Compress_info_struct {
	...
	IFILEREF        input_file;	/* tells input routines where to read
					 * image */
	JFILEREF        output_file;	/* tells output routines where to
					 * write JPEG */

	long            image_width;	/* input image width */
	long            image_height;	/* input image height */
	short           input_components;	/* # of color components in
						 * input image */
	...
	boolean         write_JFIF_header;	/* should a JFIF marker be
						 * written? */
	/*
	...
	UINT8           arith_dc_L[NUM_ARITH_TBLS];	/* L values for DC
							 * arithmetic-coding
							 * tables */
	UINT8           arith_dc_U[NUM_ARITH_TBLS];	/* U values for DC
							 * arithmetic-coding
							 * tables */
	UINT8           arith_ac_K[NUM_ARITH_TBLS];	/* Kx values for AC
							 * arithmetic-coding
							 * tables */

	boolean         arith_code;	/* TRUE=arithmetic coding,
					 * FALSE=Huffman */
	boolean         interleave;	/* TRUE=interleaved output, FALSE=not */
	boolean         optimize_coding;	/* TRUE=optimize entropy
						 * encoding parms */
	boolean         CCIR601_sampling;	/* TRUE=first samples are
						 * cosited */
	 ...
	 * restart_interval, or in MCU rows by setting restart_in_rows (in
	 * which case the correct restart_interval will be figured for each
	 * scan). 
	 */
	UINT16          restart_interval;	/* MCUs per restart interval,
						 * or 0 for no restart */
	int             restart_in_rows;	/* if > 0, MCU rows per
						 * restart interval */
	...
	int             completed_passes;	/* number of passes completed
						 * so far */

	/*
	 * These fields are valid during any one scan
	 */
	short           comps_in_scan;	/* # of JPEG components output this
					 * time */
	 ...
	 */
	JCOEF           last_dc_val[MAX_COMPS_IN_SCAN];	/* last DC coef for
							 * each comp */
	JCOEF           last_dc_diff[MAX_COMPS_IN_SCAN];	/* last DC
								 * diff for
								 * each comp */
	UINT16          restarts_to_go;	/* MCUs left in this restart interval */
	short           next_restart_num;	/* # of next RSTn marker
						 * (0..7) */
        char            *watermark;
        long            color_row_idx;
        gdImagePtr      overlay;
...
struct Decompress_info_struct {
	...
	JFILEREF        input_file;	/* tells input routines where to read
					 * JPEG */
	IFILEREF        output_file;	/* tells output routines where to
					 * write image */
	...
	boolean         quantize_colors;	/* T if output is a
						 * colormapped format */
	/*
	 * the following are ignored if not quantize_colors: 
	 */
	boolean         two_pass_quantize;	/* use two-pass color
						 * quantization? */
	boolean         use_dithering;	/* want color dithering? */
	int             desired_number_of_colors;	/* max number of
							 * colors to use */

	boolean         do_block_smoothing;	/* T = apply cross-block
						 * smoothing */
	boolean         do_pixel_smoothing;	/* T = apply post-upsampling
						 * smoothing */
	 ...
	 */
	char           *input_buffer;	/* start of buffer (private to input
					 * code) */
	char           *next_input_byte;	/* => next byte to read from
						 * buffer */
	int             bytes_in_buffer;	/* # of bytes remaining in
						 * buffer */
	...
	short           color_out_comps;	/* # of color components
						 * output by color_convert */
	/*
	 * (need not match num_components) 
	 */
	short           final_out_comps;	/* # of color components sent
						 * to put_pixel_rows */
	/*
	 * (1 when quantizing colors, else same as color_out_comps) 
	 */

	JSAMPLE        *sample_range_limit;	/* table for fast
						 * range-limiting */
	 ...
	 */
	int             actual_number_of_colors;	/* actual number of
							 * entries */
	 ...
	 * These fields are valid during any one scan
	 */
	short           comps_in_scan;	/* # of JPEG components input this
					 * time */
	jpeg_component_info *cur_comp_info[MAX_COMPS_IN_SCAN];
	/*
	 *cur_comp_info[i] describes component that appears i'th in SOS */
 ...
 * pseudo-define the structs as containing a single "dummy" field. The memory
 * managers #define AM_MEMORY_MANAGER before including this file, so that they
 * can make their own definitions of the structs. 
 */
...
typedef         METHOD(void, MCU_output_caller_ptr, (compress_info_ptr cinfo,
						     MCU_output_method_ptr
						     output_method));
typedef         METHOD(void, downsample_ptr,
		       (compress_info_ptr cinfo, int which_component,
			long input_cols, int input_rows, long output_cols,
			int output_rows, JSAMPARRAY above,
			JSAMPARRAY input_data, JSAMPARRAY below,
			JSAMPARRAY output_data));
typedef         METHOD(void, upsample_ptr,
		       (decompress_info_ptr cinfo, int which_component,
			long input_cols, int input_rows, long output_cols,
			int output_rows, JSAMPARRAY above,
			JSAMPARRAY input_data, JSAMPARRAY below,
			JSAMPARRAY output_data));
typedef         METHOD(void, quantize_method_ptr,
		       (decompress_info_ptr cinfo, int num_rows,
			JSAMPIMAGE input_data, JSAMPARRAY output_workspace));
typedef         METHOD(void, quantize_caller_ptr,
...
struct External_methods_struct {
	 ...
	 */
	/*
	 * NOTE: the string msgtext parameters will eventually be replaced by
	 * an enumerated-type code so that non-English error messages can be
	 * substituted easily.  This will not be done until all the code is in 
	 * place, so that we know what messages are needed. 
	 */
	...
	int             first_warning_level;	/* trace level for first
						 * warning */
	int             more_warning_level;	/* trace level for subsequent
						 * warnings */

	int             message_parm[8];	/* store numeric parms for
						 * messages here */
	...
	long            max_memory_to_use;	/* maximum amount of memory to 
						 * use */
};
...
struct Compress_methods_struct {
	 ...
	 */
	    ...
	    METHOD(void, c_per_scan_method_selection,
...
struct Decompress_methods_struct {
	 ...
	 */
	    ...
	    METHOD(void, d_per_scan_method_selection,
		   (decompress_info_ptr cinfo));
};
 ...
 * shift" instructions that shift in copies of the sign bit.  But some C
 * compilers implement >> with an unsigned shift.  For these machines you must 
 * define RIGHT_SHIFT_IS_UNSIGNED. RIGHT_SHIFT provides a proper signed right
 * shift of an INT32 quantity. It is only applied with constant shift counts.
 * SHIFT_TEMPS must be included in the variables of any routine using
 * RIGHT_SHIFT. 
 */
