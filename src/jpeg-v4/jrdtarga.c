...
static int      dup_pixel_count;	/* # of times to duplicate previous
					 * pixel */
 ...
 */
{
	...
	if (--block_count < 0) {	/* decrement pixels remaining in block 
		...
		if (i & 0x80) {	/* Start of duplicate-pixel block? */
			dup_pixel_count = i & 0x7F;	/* number of
							 * duplications after
							 * this one */
			block_count = 0;	/* then read new block header */
		} else {
			block_count = i & 0x7F;	/* number of pixels after this 
						 * one */
		}
	}
 ...
 */
{
	...
	for (col = cinfo->image_width; col > 0; col--) {
		...
		t += UCH(tga_pixel[1]) << 8;
		/*
		 * We expand 5 bit data to 8 bit sample width. The format of
		 * the 16-bit (LSB first) input word is xRRRRRGGGGGBBBBB 
		 */
 ...
 */
{
	...
	for (col = cinfo->image_width; col > 0; col--) {
		(*read_pixel) (cinfo);	/* Load next pixel into tga_pixel */
		*ptr0++ = (JSAMPLE) UCH(tga_pixel[2]);	/* convert BGR to RGB
							 * order */
...
input_init(compress_info_ptr cinfo)
{
	...
	int             idlen, cmaptype, subtype, flags, interlace_type,
	    components;
...
#define GET_2B(offset)	((unsigned int) UCH(targaheader[offset]) + \
	...
	/*
	 * Pretend "15-bit" pixels are 16-bit --- we ignore attribute bit
	 * anyway 
	 */
	...
	if (cmaptype > 1 ||	/* cmaptype must be 0 or 1 */
	    pixel_size < 1 || pixel_size > 4 || (UCH(targaheader[16]) & 7) != 0 ||	/* bits/pixel 
											 * must 
											 * be 
											 * multiple 
											 * of 
											 * 8 
											 */
	    interlace_type != 0)	/* currently don't allow interlaced
					 * image */
