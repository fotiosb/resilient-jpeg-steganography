...
smooth_coefficients(decompress_info_ptr cinfo,
		    ...
		    JBLOCKROW above,
		    JBLOCKROW currow, JBLOCKROW below, JBLOCKROW output)
{
	...
	/*
	 * First, copy the block row as-is. This takes care of the first &
	 * last blocks in the row, the top/bottom special cases, and the
	 * higher-order coefficients in each block. 
	 */
	...
	if (above != NULL && below != NULL) {
		for (col = 1; col < blocks_in_row - 1; col++) {
			...
			/*
			 * See section K.8 of the JPEG standard. As I
			 * understand it, this produces approximations for the 
			 * low frequency AC components, based on the DC values 
			 * of the block and its eight neighboring blocks.
			 * (Thus it can't be used for blocks on the image
			 * edges.) 
			 */

			/*
			 * The layout of these variables corresponds to text
			 * and figure in K.8 
			 */
