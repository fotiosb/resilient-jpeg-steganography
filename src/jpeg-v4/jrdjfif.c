 ...
 */
{
	...
	while (length > 0) {
		...
		TRACEMS8(cinfo->emethods, 2,
			 "        %3d %3d %3d %3d %3d %3d %3d %3d", bits[1],
			 bits[2], bits[3], bits[4], bits[5], bits[6], bits[7],
			 bits[8]);
		TRACEMS8(cinfo->emethods, 2,
			 "        %3d %3d %3d %3d %3d %3d %3d %3d", bits[9],
 ...
 */
{
	...
	while (length > 0) {
		...
		if (index >= NUM_ARITH_TBLS) {	/* define AC table */
			cinfo->arith_ac_K[index - NUM_ARITH_TBLS] =
			    (UINT8) val;
		} else {	/* define DC table */
			cinfo->arith_dc_L[index] = (UINT8) (val & 0x0F);
			cinfo->arith_dc_U[index] = (UINT8) (val >> 4);
			if (cinfo->arith_dc_L[index] >
			    cinfo->arith_dc_U[index])
				ERREXIT1(cinfo->emethods,
					 "Bogus DAC value 0x%x", val);
		}
 ...
 */
{
	...
	while (length > 0) {
		...
		TRACEMS2(cinfo->emethods, 1,
			 "Define Quantization Table %d  precision %d", n,
			 prec);
...
#define JFIF_LEN 14
	...
	if (length >= JFIF_LEN) {
		...
		if (b[0] == 0x4A && b[1] == 0x46 && b[2] == 0x49
			 ...
			 */
			if (b[5] != 1)
				ERREXIT2(cinfo->emethods,
					 "Unsupported JFIF revision number %d.%02d",
					 b[5], b[6]);
			/*
			 * Minor version should be 0..2, but try to process
			 * anyway if newer 
			 */
			if (b[6] > 2)
				TRACEMS2(cinfo->emethods, 1,
			...
			/*
			 * Assume colorspace is YCbCr, unless UI has
			 * overridden me 
			 */
 ...
 */
{
	...
	cinfo->comp_info =
	    (jpeg_component_info *) (*cinfo->emethods->alloc_small)
 ...
 */
{
	...
	do {
		...
		do {		/* skip any duplicate FFs */
			/*
			 * we don't increment nbytes here since extra FFs are
			 * legal 
			 */
			c = JGETC(cinfo);
		} while (c == 0xFF);
...
read_file_header(decompress_info_ptr cinfo)
{
	 ...
	 * probably not a JPEG file at all.  If the user interface wants to
	 * support nonstandard headers in front of the SOI, it must skip over
	 * them itself before calling jpeg_decompress(). 
	 */
	 ...
	 */
	/*
	 * (too bad the JPEG committee didn't provide a real way to specify
	 * this) 
	 */
	...
	case 3:
		...
		if (cinfo->jpeg_color_space == CS_UNKNOWN) {
			...
			if (cid0 == 1 && cid1 == 2 && cid2 == 3)
				cinfo->jpeg_color_space = CS_YCbCr;	/* assume 
									 * it's 
									 * JFIF 
									 * w/out 
									 * marker 
				...
				cinfo->jpeg_color_space = CS_YIQ;	/* prototype's 
									 * YIQ 
									 * matrix 
									 */
			else {
	...
	for (;;) {
		if (marker < M_SOF0)
			...
			if (marker == (M_RST0 + ((desired + 1) & 7)) ||
				...
				action = 1;	/* desired restart or too far
						 * away */
		}
		TRACEMS2(cinfo->emethods, 4,
			 "At marker 0x%02x, recovery action %d", marker,
			 action);
		switch (action) {
		case 1:
			/*
...
jselrjfif(decompress_info_ptr cinfo)
{
	...
	/*
	 * For JFIF/raw-JPEG format, the user interface supplies
	 * read_jpeg_data. 
	 */
#if 0
