...
{
	INT32           i, x2;
	SHIFT_TEMPS Cr_r_tab = (int *) (*cinfo->emethods->alloc_small)
	...
	for (i = 0; i <= MAXJSAMPLE; i++) {
		 ...
		 */
		/*
		 * We also add in ONE_HALF so that need not do it in inner
		 * loop 
		 */
		...
		JSAMPIMAGE input_data, JSAMPIMAGE output_data)
{
	...
	SHIFT_TEMPS for (row = 0; row < num_rows; row++) {
		...
		for (col = 0; col < num_cols; col++) {
			...
			/*
			 * Note: if the inputs were computed directly from RGB 
			 * values, range-limiting would be unnecessary here;
			 * but due to possible noise in the DCT/IDCT phase, we 
			 * do need to apply range limits. 
			 */
...
jseldcolor(decompress_info_ptr cinfo)
{
	...
	default:
		/*
		 * Permit null conversion from CMYK or YCbCr to same output
		 * space 
		 */
	...
	if (cinfo->quantize_colors)
		cinfo->final_out_comps = 1;	/* single colormapped output
						 * component */
	else
