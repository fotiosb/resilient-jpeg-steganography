...
#ifdef D_MULTISCAN_FILES_SUPPORTED	/* wish we could assume ANSI's
					 * defined() */
...
static JSAMPIMAGE fullsize_ptrs;	/* workspace for access_big_sarray()
					 * result */
#endif
 ...
 */
{
	...
	for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
		...
		compptr->downsampled_width =
		    jround_up(compptr->true_comp_width,
			      (long) (compptr->MCU_width * DCTSIZE));
		compptr->downsampled_height =
		    jround_up(compptr->true_comp_height,
 ...
 */
{
	...
	for (ci = 0; ci < num_comps; ci++) {
		image[ci] =
		    (*cinfo->emethods->alloc_small_sarray) (num_cols,
							    num_rows);
	}
	return image;
}
 ...
 */
{
	...
	for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
		vs = cinfo->cur_comp_info[ci]->v_samp_factor;	/* row group
								 * height */
		/*
		 ...
		 */
		sampled_data[1][ci] =
		    (JSAMPARRAY) (*cinfo->emethods->alloc_small)
		 ...
		 */
		for (i = 0; i < vs * 2; i++) {
			sampled_data[1][ci][vs * DCTSIZE + i] =
			    sampled_data[0][ci][vs * (DCTSIZE - 2) + i];
			sampled_data[1][ci][vs * (DCTSIZE - 2) + i] =
			    sampled_data[0][ci][vs * DCTSIZE + i];
		}
	}
}
 ...
 */
{
	...
	for (i = 0; i <= MAXJSAMPLE; i++) {
		...
		table[i + (MAXJSAMPLE + 1)] = (JSAMPLE) i;	/* sample_range_limit[x] 
								 * = x */
		table[i + (MAXJSAMPLE + 1) * 2] = MAXJSAMPLE;	/* x beyond
								 * MAXJSAMPLE */
	}
}
 ...
 */
{
	...
	for (row = 1; row <= num_rows; row++) {
		jcopy_sample_rows(image_data, source_row, image_data,
				  source_row + row, 1, num_cols);
	}
}
...
expand(decompress_info_ptr cinfo,
       ...
       long fullsize_width, short above, short current, short below, short out)
/*
 ...
 */
{
	...
	JSAMPROW        dummy[MAX_SAMP_FACTOR];	/* for downsample expansion at 
						 * top/bottom */
	short           ci, vs, i;
	...
	for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
		...
		if (above >= 0)
			...
			/*
			 * Top of image: make a dummy above-context with
			 * copies of 1st row 
			 */
			/*
			...
			above_ptr = (JSAMPARRAY) dummy;	/* possible near->far
							 * pointer conv */
		}
		...
		if (below >= 0)
			...
			/*
			 * Bot of image: make a dummy below-context with
			 * copies of last row 
			 */
			for (i = 0; i < vs; i++)
				dummy[i] =
				    sampled_data[ci][(current + 1) * vs - 1];
			below_ptr = (JSAMPARRAY) dummy;	/* possible near->far
							 * pointer conv */
		}
 ...
 */
{
	...
	for (pixel_rows_output = 0; pixel_rows_output < cinfo->image_height;
		...
		for (ci = 0; ci < cinfo->num_components; ci++) {
			fullsize_ptrs[ci] =
			    (*cinfo->emethods->access_big_sarray)
			    (fullsize_image[ci], pixel_rows_output, FALSE);
		}
		/*
		 * Let the quantizer have its way with the data. Note that
		 * output_workspace is simply workspace for the quantizer;
		 * when it's ready to output, it must call put_pixel_rows
		 * itself. 
		 */
 ...
 */
{
	...
	/*
	 * Special case for top of image: need to pre-fetch a row & init
	 * whichb 
	 */
	if (cur_mcu_row == 0) {
		...
		if (cinfo->MCU_rows_in_scan > 1) {
			...
			smooth_mcu_row(cinfo, (JBLOCKIMAGE) NULL, bsmooth[0],
				       bsmooth[1], coeff_data);
		} else {
			smooth_mcu_row(cinfo, (JBLOCKIMAGE) NULL, bsmooth[0],
				       (JBLOCKIMAGE) NULL, coeff_data);
		}
	...
	*whichb = next;		/* advance whichb for next time */
	...
	if (cur_mcu_row >= cinfo->MCU_rows_in_scan - 1) {
		smooth_mcu_row(cinfo, bsmooth[prev], bsmooth[cur],
			       (JBLOCKIMAGE) NULL, coeff_data);
		return;
	}
...
{
	long            fullsize_width;	/* # of samples per row in full-size
					 * buffers */
	long            cur_mcu_row;	/* counts # of MCU rows processed */
	long            pixel_rows_output;	/* # of pixel rows actually
						 * emitted */
	int             mcu_rows_per_loop;	/* # of MCU rows processed per 
						 * outer loop */
	/*
...
#endif
	/*
	 * Work buffer for downsampled image data (see comments at head of
	 * file) 
	 */
	...
	/*
	 * Tell the memory manager to instantiate big arrays. We don't need
	 * any big arrays in this controller, but some other module (like the
	 * output file writer) may need one. 
	 */
	...
	/*
	 * Loop over scan's data: rows_in_mem pixel rows are processed per
	 * loop 
	 */
	...
	for (cur_mcu_row = 0; cur_mcu_row < cinfo->MCU_rows_in_scan;
		...
		/*
		 * Obtain v_samp_factor block rows of each component in the
		 * scan. 
		 */
		/*
		 * This is a single MCU row if interleaved, multiple MCU rows
		 * if not. 
		 */
		/*
		 * In the noninterleaved case there might be fewer than
		 * v_samp_factor 
		 */
		/*
		 * block rows remaining; if so, pad with copies of the last
		 * pixel row 
		 */
		/*
		 * so that upsampling doesn't have to treat it as a special
		 * case. 
		 */
		...
		for (ri = 0; ri < mcu_rows_per_loop; ri++) {
			if (cur_mcu_row + ri < cinfo->MCU_rows_in_scan) {
...
#endif
					(*cinfo->methods->
					 disassemble_MCU) (cinfo, coeff_data);

				(*cinfo->methods->reverse_DCT) (cinfo,
								coeff_data,
								sampled_data
								[whichss],
								ri * DCTSIZE);
				...
				/*
				 * Need to pad out with copies of the last
				 * downsampled row. 
				 */
				/*
				 * This can only happen if there is just one
				 * component. 
				 */
		...
		if (cur_mcu_row) {
			 ...
			 */
			expand(cinfo, sampled_data[whichss], fullsize_data,
			       fullsize_width, (short) DCTSIZE,
			       (short) (DCTSIZE + 1), (short) 0,
			       (short) (DCTSIZE - 1));
			/*
			 * and dump the previous set's expanded data 
			 */
			emit_1pass(cinfo, rows_in_mem, fullsize_data,
				   (JSAMPARRAY) NULL);
			pixel_rows_output += rows_in_mem;
			/*
			 ...
			 */
			expand(cinfo, sampled_data[whichss], fullsize_data,
			       fullsize_width, (short) (DCTSIZE + 1),
			 ...
			 */
			expand(cinfo, sampled_data[whichss], fullsize_data,
			       fullsize_width, (short) (-1), (short) 0,
			       (short) 1, (short) 0);
		}
		/*
		...
		for (i = 1; i <= DCTSIZE - 2; i++) {
			expand(cinfo, sampled_data[whichss], fullsize_data,
			       fullsize_width, (short) (i - 1), (short) i,
			       (short) (i + 1), (short) i);
		}
	}			/* end of outer loop */
...
complex_dcontroller(decompress_info_ptr cinfo)
{
	...
	(*cinfo->emethods->alloc_big_arrays)
	    ...
	    ((long) (fullsize_width	/* max width in samples */
	     ...
	     (long) ((fullsize_width / DCTSIZE)	/* max width in blocks */
		     ...
		     * cinfo->num_components	/* max components per scan */
		     * (cinfo->do_block_smoothing ? 4 : 1)),	/* how many of 
								 * these we
								 * need */
	     /*
	...
	/*
	 * If file is single-scan, we can do color quantization prescan
	 * on-the-fly during the scan (we must be doing 2-pass quantization,
	 * else this method would not have been selected).  If it is multiple
	 * scans, we have to make a separate pass after we've collected all
	 * the components.  (We could save some I/O by doing CQ prescan during 
	 * the last scan, but the extra logic doesn't seem worth the trouble.) 
	 */

	single_scan = (cinfo->comps_in_scan == cinfo->num_components);

	/*
	 * Account for passes needed (color quantizer adds its passes
	 * separately). If multiscan file, we guess that each component has
	 * its own scan, and increment completed_passes by the number of
	...
	if (single_scan)
		...
		cinfo->total_passes += cinfo->num_components;	/* guessed #
								 * of scans */
		if (cinfo->two_pass_quantize)
			cinfo->total_passes++;	/* account for separate CQ
						 * prescan pass */
	}
	if (!cinfo->two_pass_quantize)
		cinfo->total_passes++;	/* count output pass unless quantizer
					 * does it */
	...
	do {
		...
		if (cinfo->comps_in_scan == 1) {
			...
			interleaved_scan_setup(cinfo);
			/*
			 * in an interleaved scan, one MCU row provides Vk
			 * block rows 
			 */
			mcu_rows_per_loop = 1;
		}
		...
		coeff_data = alloc_MCU_row(cinfo);
		/*
		 * if doing cross-block smoothing, need extra space for its
		 * input 
		 */
#ifdef BLOCK_SMOOTHING_SUPPORTED
		if (cinfo->do_block_smoothing) {
		...
		for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
			fullsize_ptrs[ci] =
			    (*cinfo->emethods->access_big_sarray)
			    (fullsize_image
			     [cinfo->cur_comp_info[ci]->component_index],
			     (long) 0, TRUE);
		}
		...
		/*
		 * Loop over scan's data: rows_in_mem pixel rows are processed 
		 * per loop 
		 */
		...
		for (cur_mcu_row = 0; cur_mcu_row < cinfo->MCU_rows_in_scan;
		     cur_mcu_row += mcu_rows_per_loop) {
			(*cinfo->methods->progress_monitor) (cinfo,
							     cur_mcu_row,
							     cinfo->
							     MCU_rows_in_scan);

			whichss ^= 1;	/* switch to other downsampled-data
					 * buffer */

			/*
			 * Obtain v_samp_factor block rows of each component
			 * in the scan. 
			 */
			/*
			 * This is a single MCU row if interleaved, multiple
			 * MCU rows if not. 
			 */
			/*
			 * In the noninterleaved case there might be fewer
			 * than v_samp_factor 
			 */
			/*
			 * block rows remaining; if so, pad with copies of the 
			 * last pixel row 
			 */
			/*
			 * so that upsampling doesn't have to treat it as a
			 * special case. 
			 */
			...
			for (ri = 0; ri < mcu_rows_per_loop; ri++) {
				if (cur_mcu_row + ri < cinfo->MCU_rows_in_scan) {
					...
					/*
					 * Need to pad out with copies of the
			...
			if (cur_mcu_row) {
				 ...
				 * Expand last row group of previous set 
				 */
				expand(cinfo, sampled_data[whichss],
				       fullsize_ptrs, fullsize_width,
				       (short) DCTSIZE, (short) (DCTSIZE + 1),
				       (short) 0, (short) (DCTSIZE - 1));
				/*
				 * If single scan, can do color quantization
				 * prescan on-the-fly 
				 */
				if (single_scan)
					(*cinfo->methods->
					 color_quant_prescan) (cinfo,
							       rows_in_mem,
							       fullsize_ptrs,
							       output_workspace
							       [0]);
				...
				/*
				 * Expand first row group with dummy
				 * above-context 
				 */
				expand(cinfo, sampled_data[whichss],
				       fullsize_ptrs, fullsize_width,
			...
			/*
			 * Expand second through next-to-last row groups of
			 * this set 
			 */
			for (i = 1; i <= DCTSIZE - 2; i++) {
				expand(cinfo, sampled_data[whichss],
		 ...
		 * Note whichss points to last buffer side used 
		 */
		expand(cinfo, sampled_data[whichss], fullsize_ptrs,
		       fullsize_width, (short) (DCTSIZE - 2),
		       (short) (DCTSIZE - 1), (short) (-1),
		       (short) (DCTSIZE - 1));
		/*
		 * If single scan, finish on-the-fly color quantization
		 * prescan 
		 */
		if (single_scan)
		 ...
		 */
		 ...
		 && (*cinfo->methods->read_scan_header) (cinfo));
	...
	if (single_scan) {
		...
		/*
		 * For multiple-scan file, do the CQ prescan as a separate
		 * pass. 
		 */
		/*
		 * The main reason why prescan is passed the output_workspace
		 * is 
		 */
		/*
		 ...
		 */
		if (cinfo->two_pass_quantize)
			scan_big_image(cinfo,
				       cinfo->methods->color_quant_prescan);
	}
...
jseldpipeline(decompress_info_ptr cinfo)
{
	...
	if (cinfo->comps_in_scan == cinfo->num_components) {
		...
		if (cinfo->two_pass_quantize) {
#ifdef NEED_COMPLEX_CONTROLLER
			cinfo->methods->d_pipeline_controller =
			    complex_dcontroller;
#else
			ERREXIT(cinfo->emethods,
				"2-pass quantization support was not compiled");
#endif
		} else
			cinfo->methods->d_pipeline_controller =
			    simple_dcontroller;
...
#else
		ERREXIT(cinfo->emethods,
			"Multiple-scan support was not compiled");
#endif
	}
}
