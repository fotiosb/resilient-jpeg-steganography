...
/*
 * Define this if an ordinary "char" type is unsigned. If you're not sure,
 * leaving it undefined will work at some cost in speed. If you defined
 * HAVE_UNSIGNED_CHAR then it doesn't matter very much. 
 */
 ...
 * (unsigned) shift; leave it undefined if ">>" is a signed (arithmetic)
 * shift, which is the normal and rational definition. 
 */
...
/*
 * Define this if your system needs explicit cleanup of temporary files. This
 * is crucial under MS-DOS, where the temporary "files" may be areas of
 * extended memory; on most other systems it's not as important. 
 */
...
/*
 * If you're getting bored, that's the end of the symbols you HAVE to worry
 * about.  Go fix the makefile and compile. 
 */


/*
 * If your compiler supports inline functions, define INLINE as the inline
 * keyword; otherwise define it as empty. 
 */
...
/*
 * On a few systems, type boolean and/or macros FALSE, TRUE may appear in
 * standard header files.  Or you may have conflicts with application-
...
/*
 * This defines the size of the I/O buffers for entropy compression and
 * decompression; you could reduce it if memory is tight. 
 */
 ...
 * #define RLE_SUPPORTED 
				 *//*
				 * RLE image file format (by default, no) 
				 */
...
/*
 * JSAMPLE should be the smallest type that will hold the values 0..255. You
 * can use a signed char by having GETJSAMPLE mask it with 0xFF. If you have
 * only signed chars, and you are more worried about speed than memory usage,
 * it might be a win to make JSAMPLE be short. 
 */
...
/*
 * If int is 32 bits this'll be horrendously inefficient storage-wise. But
 * since we don't actually support 16-bit samples (ie lossless coding) yet,
...
/*
 * Here we define the representation of a DCT frequency coefficient. This
 * should be a signed 16-bit value; "short" is usually right. It's important
 * that this be exactly 16 bits, no more and no less; more will cost you a BIG 
 * hit of memory, less will give wrong answers. 
 */
 ...
 * They must be at least as wide as specified; but making them too big won't
 * cost a huge amount of memory, so we don't provide special extraction code
 * like we did for JSAMPLE.  (In other words, these typedefs live at a
 * different point on the speed/space tradeoff curve.) 
 */
