 ...
 */
{
	...
	if (cinfo->out_color_space == CS_GRAYSCALE) {
		targaheader[2] = 3;	/* image type = uncompressed
					 * gray-scale */
		targaheader[16] = 8;	/* bits per pixel */
	} else {		/* must be RGB */
		if (num_colors > 0) {
			targaheader[2] = 1;	/* image type = colormapped
						 * RGB */
			targaheader[16] = 8;
		} else {
			targaheader[2] = 2;	/* image type = uncompressed
						 * RGB */
			targaheader[16] = 24;
		}
	}
...
output_init(decompress_info_ptr cinfo)
{
	if (cinfo->out_color_space == CS_GRAYSCALE) {
		...
		/*
		 * For quantized output, defer writing header until
		 * put_color_map time. 
		 */
 ...
 */
{
	...
	for (row = 0; row < num_rows; row++) {
		...
		for (col = width; col > 0; col--) {
			putc(GETJSAMPLE(*ptr2), outfile);	/* write in
								 * BGR order */
			ptr2++;
...
METHODDEF void
put_gray_rows(decompress_info_ptr cinfo, int num_rows, JSAMPIMAGE pixel_data)
/*
 ...
 */
{
	...
	for (row = 0; row < num_rows; row++) {
		...
		for (col = width; col > 0; col--) {
			*row_bufferptr++ = (char) GETJSAMPLE(*ptr2++);	/* BGR 
									 * order 
									 */
		  ...
		  JSAMPIMAGE pixel_data)
{
	...
	for (row = 0; row < num_rows; row++) {
		...
		for (col = width; col > 0; col--) {
			pixval = GETJSAMPLE(*ptr++);
			*row_bufferptr++ =
			    (char) GETJSAMPLE(color_map0[pixval]);
		}
		(void) JFWRITE(outfile, row_buffer, width);
	}
}
...
put_color_map(decompress_info_ptr cinfo, int num_colors, JSAMPARRAY colormap)
{
	...
	if (cinfo->out_color_space == CS_RGB) {
		...
		if (num_colors > 256)
			ERREXIT(cinfo->emethods,
				"Too many colors for Targa output");
		/*
