...
/*
 * These are the amounts to shift an input value to get a histogram index. For 
 * a combination 8/12 bit implementation, would need variables here... 
 */
 ...
 */
{
	...
	/*
	 * We use 2-norm rather than real volume here. Some care is needed
	 * since the differences are expressed in histogram-cell units; if
	 * HIST_Y_BITS != HIST_C_BITS, we have to adjust the scaling to get
	 * the proper scaled-YCbCr-space distance. This code won't work right
	 * if HIST_Y_BITS < HIST_C_BITS, but that shouldn't ever be true. Note 
	 * norm > 0 iff box is splittable, so need not check separately. 
	 */

	for (i = 0, boxp = boxlist; i < numboxes; i++, boxp++) {
		c0 = (boxp->c0max - boxp->c0min) * Y_SCALE;
		c1 = (boxp->c1max - boxp->c1min) << (HIST_Y_BITS -
						     HIST_C_BITS);
		c2 = (boxp->c2max - boxp->c2min) << (HIST_Y_BITS -
						     HIST_C_BITS);
 ...
 */
{
	...
	while (numboxes < desired_colors) {
		 ...
		 * longest scaled axis. See notes in find_biggest_volume about 
		 * scaling... 
		 */
		 ...
		 * bounds. Current algorithm: split at halfway point. (Since
		 * the box has been shrunk to minimum volume, any split will
		 * produce two nonempty subboxes.) Note that lb value is max
		 * for lower box, so must be < old max. 
		 */
		switch (n) {
		case 0:
...
{
	/*
	 * This requires a little trickery since color_convert expects to deal 
	 * with 3-D arrays (a 2-D sample array for each component). We must
	 * promote the colormaps into one-row 3-D arrays. 
	 */
	short           ci;
	JSAMPARRAY      input_hack[3];
	JSAMPARRAY      output_hack[10];	/* assume no more than 10
						 * output components */
	...
	(*cinfo->methods->color_convert) (cinfo, 1,
					  (long) cinfo->
					  actual_number_of_colors, input_hack,
					  output_hack);
}
 ...
 */
{
	 ...
	 */
	boxlist =
	    (boxptr) (*cinfo->emethods->alloc_small) (desired * SIZEOF(box));
	/*
...
#define BOX_Y_LOG  (HIST_Y_BITS-3)	/* log2(hist cells in update box, Y
					 * axis) */
#endif
#ifndef BOX_C_LOG		/* so you can override from Makefile */
#define BOX_C_LOG  (HIST_C_BITS-3)	/* log2(hist cells in update box, C
					 * axes) */
#endif
 ...
 * for the nearest entry to some cell(s) in the update box.  The update box is 
 * specified by the center coordinates of its first cell.  The number of
 * candidate colormap entries is returned, and their colormap indexes are
 * placed in colorlist[]. This routine uses Heckbert's "locally sorted search" 
 * criterion to select the colors that need further consideration. 
 */
{
	...
	INT32           mindist[MAXNUMCOLORS];	/* min distance to colormap
						 * entry i */

	/*
	 * Compute true coordinates of update box's upper corner and center.
	 * Actually we compute the coordinates of the center of the
	 * upper-corner histogram cell, which are the upper bounds of the
	 * volume we care about. Note that since ">>" rounds down, the
	 * "center" values may be closer to min than to max; hence comparisons 
	 * to them must be "<=", not "<". 
	 */
	 ...
	 * only the smallest maximum distance is of interest. Note we have to
	 * scale Y to get correct distance in scaled space. 
	 */
...
/*
 * Find the closest colormap entry for each cell in the update box, given the
 * list of candidate colors prepared by find_nearby_colors. Return the indexes 
 * of the closest entries in the bestcolor[] array. This routine uses Thomas'
 * incremental distance calculation method to find the distance from a
 * colormap entry to successive cells in the box. 
 */
{
	int             ic0, ic1, ic2;
	int             i, icolor;
	...
	register INT32  xx2;
	INT32           inc0, inc1, inc2;	/* initial values for
						 * increments */
	/*
	 ...
	 * best-so-far, update best distance and color number. Note we have to 
	 * scale Y to get correct distance in scaled space. 
	 */
...
#define STEP_C  (1 << C_SHIFT)
	...
	for (i = 0; i < numcolors; i++) {
		...
		inc0 =
		    (minc0 -
		     (int) GETJSAMPLE(my_colormap[0][icolor])) * Y_SCALE;
		dist0 = inc0 * inc0;
		...
		/*
		 * Now loop over all cells in box, updating distance per
		 * Thomas method 
		 */
		bptr = bestdist;
		cptr = bestcolor;
 ...
 */
{
	int             minc0, minc1, minc2;	/* lower left corner of update 
						 * box */
	int             ic0, ic1, ic2;
...
/*
 * Declarations for Floyd-Steinberg dithering. Errors are accumulated into
 * the arrays evenrowerrs[] and oddrowerrs[]. These have resolutions of 1/16th 
 * of a pixel count.  The error at a given pixel is propagated to its
 * unprocessed neighbors using the standard F-S fractions, ... (here) 7/16
 * 3/16 5/16 1/16 We work left-to-right on even rows, right-to-left on odd
 * rows. Each of the arrays has (#columns + 2) entries; the extra entry at
 * each end saves us from special-casing the first and last pixels. Each entry 
 * is three values long. In evenrowerrs[], the entries for a component are
 * stored left-to-right, but in oddrowerrs[] they are stored right-to-left.
 * This means we always process the current row's error entries in increasing
 * order and the next row's error entries in decreasing order, regardless of
 * whether we are working L-to-R or R-to-L in the pixel data! Note: on a wide 
 * image, we might not have enough room in a PC's near data segment to hold
 * the error arrays; so they are allocated with alloc_medium. 
 */
...
static FSERRPTR evenrowerrs, oddrowerrs;	/* current-row and next-row
						 * errors */
 ...
 */
{
	...
	SHIFT_TEMPS
	    /*
	     * Convert data to colormap indexes, which we save in
	     * output_workspace 
	     */
	    for (row = 0; row < num_rows; row++) {
		ptr0 = image_data[0][row];
		...
		for (col = width; col > 0; col--) {
			/*
			 * For each component, get accumulated error and round 
			 * to integer; form pixel value + error, and
			 * range-limit to 0..MAXJSAMPLE. RIGHT_SHIFT rounds
			 * towards minus infinity, so adding 8 is correct for
			 * either sign of the error value.  Max error is +-
			 * MAXJSAMPLE. 
			 */
			 ...
			 */
			cachep =
			    &histogram[c0 >> Y_SHIFT][c1 >> C_SHIFT][c2 >>
								     C_SHIFT];
			/*
			 * If we have not seen this color before, find nearest 
			 * colormap 
			 ...
			 */
			if (*cachep == 0)
				fill_inverse_cmap(cinfo, c0 >> Y_SHIFT,
						  c1 >> C_SHIFT,
						  c2 >> C_SHIFT);
			/*
			 * Now emit the colormap index for this cell 
			...
			*outptr = (JSAMPLE) pixcode;
			 ...
			 */
			/*
			 * Remember that nextrowerr entries are in reverse
			 * order! 
			 */
			two_val = c0 * 2;
			nextrowerr[0 - 3] = c0;	/* not +=, since not
						 * initialized yet */
			...
			thisrowerr[0 + 3] += c0;
			two_val = c1 * 2;
			nextrowerr[1 - 3] = c1;	/* not +=, since not
						 * initialized yet */
			...
			thisrowerr[1 + 3] += c1;
			two_val = c2 * 2;
			nextrowerr[2 - 3] = c2;	/* not +=, since not
						 * initialized yet */
			...
			ptr2 += dir;
			outptr += dir;
			thisrowerr += 3;	/* cur-row error ptr advances
						 * to right */
			nextrowerr -= 3;	/* next-row error ptr advances 
						 * to left */
		}
	}
	/*
...
color_quant_init(decompress_info_ptr cinfo)
{
	...
	my_colormap = (*cinfo->emethods->alloc_small_sarray)
	    ((long) cinfo->desired_number_of_colors, (long) 3);
	...
	if (cinfo->use_dithering) {
		size_t          arraysize =
		    (size_t) ((cinfo->image_width +
			       2L) * 3L * SIZEOF(FSERROR));

		evenrowerrs =
		    (FSERRPTR) (*cinfo->emethods->alloc_medium) (arraysize);
		oddrowerrs =
		    (FSERRPTR) (*cinfo->emethods->alloc_medium) (arraysize);
		/*
		 * we only need to zero the forward contribution for current
		 * row. 
		 */
...
color_quant_doit(decompress_info_ptr cinfo, quantize_caller_ptr source_method)
{
	 ...
	 */
	/*
	 * NB: the output module may continue to use the colormap until
	 * shutdown. 
	 */
	(*cinfo->methods->put_color_map) (cinfo,
