 ...
 */
{
	 ...
	 */
	/*
	 * So is output file format selection (both are done by user
	 * interface). 
	 */
	...
	if (cinfo->num_components != 3 || cinfo->jpeg_color_space != CS_YCbCr)
		cinfo->two_pass_quantize = FALSE;	/* 2-pass only handles 
							 * YCbCr input */
	if (cinfo->out_color_space == CS_GRAYSCALE)
		cinfo->two_pass_quantize = FALSE;	/* Should use 1-pass
							 * for grayscale out */
	...
	if (cinfo->quantize_colors) {
		ERREXIT(cinfo->emethods,
			"Color quantization was not compiled");
	}
#endif
#endif
	 ...
	 */
	cinfo->methods->d_per_scan_method_selection =
 ...
 */
{
	...
	for (ci = 0; ci < cinfo->num_components; ci++) {
		compptr = &cinfo->comp_info[ci];
		compptr->true_comp_width =
		    (cinfo->image_width * compptr->h_samp_factor +
		     cinfo->max_h_samp_factor - 1)
		    / cinfo->max_h_samp_factor;
		compptr->true_comp_height =
		    (cinfo->image_height * compptr->v_samp_factor +
		     cinfo->max_v_samp_factor - 1)
...
{
	/*
	 * Init pass counts to 0 --- total_passes is adjusted in method
	 * selection 
	 */
	 ...
	 * SOS marker is read now.  NOTE: the user interface must have
	 * initialized the read_file_header method pointer (eg, by calling
	 * jselrjfif or jselrtiff). The other file reading methods
	 * (read_scan_header etc.) were probably set at the same time, but
