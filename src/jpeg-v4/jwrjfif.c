 ...
 */
{
	...
	if (htbl == NULL)
		ERREXIT1(cinfo->emethods,
			 "Huffman table 0x%02x was not defined", index);
 ...
 */
{
	...
	if (cinfo->image_height > 65535L || cinfo->image_width > 65535L)
		ERREXIT(cinfo->emethods,
			"Maximum image dimension for JFIF is 65535 pixels");
...
write_file_header(compress_info_ptr cinfo)
{
	 ...
	 */
	/*
	 * If we did want to do that, we'd have a problem with checking
	 * precision 
	 */
	/*
	...
	if (cinfo->arith_code || (cinfo->data_precision != 8))
		is_baseline = FALSE;
	for (i = 0; i < cinfo->num_components; i++) {
		if (cinfo->comp_info[i].dc_tbl_no > 1
		    || cinfo->comp_info[i].ac_tbl_no > 1)
			is_baseline = FALSE;
	}
	if (prec && is_baseline) {
		is_baseline = FALSE;
	 ...
	 */
	if (cinfo->arith_code)
		emit_sof(cinfo, M_SOF9);	/* SOF code for arithmetic
						 * coding */
	else if (is_baseline)
		emit_sof(cinfo, M_SOF0);	/* SOF code for baseline
						 * implementation */
	else
...
write_scan_header(compress_info_ptr cinfo)
{
	...
	if (cinfo->arith_code) {
		/*
		 * Emit arith conditioning info.  We will have some
		 * duplication if the file has multiple scans, but it's so
		 * small it's hardly worth worrying about. 
		 */
		emit_dac(cinfo);
	} else {
		/*
		 ...
		 */
		for (i = 0; i < cinfo->comps_in_scan; i++) {
			emit_dht(cinfo, cinfo->cur_comp_info[i]->dc_tbl_no,
				 FALSE);
			emit_dht(cinfo, cinfo->cur_comp_info[i]->ac_tbl_no,
				 TRUE);
		}
	}
